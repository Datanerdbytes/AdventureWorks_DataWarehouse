/*
===================================================================================
DATA WAREHOUSE LIFECYCLE: BRONZE SCHEMA DDL ARCHITECTURE
===================================================================================
Script Blueprint:  Create Bronze Ingestion Layer Tables
Database Context:  AdventureWorksDW2022
Target Schema:     bronze
Data Architecture: Medallion Architecture (Raw Ingestion / Landing Zone)

Description:
    - This script initializes the data structures for the Bronze (landing) layer.
    - Implements a destructive DDL pattern (DROP-IF-EXISTS) to cleanly reset or 
      redefine table shapes before data migrations.
    - Sets up operational metadata infrastructure (`Pipeline_Log` and 
      `Pipeline_Watermarks`) to support transaction transparency, error handling, 
      and high-velocity delta logic tracking.

Metadata & Ingestion Design Patterns:
    - Incremental Tracking: High-fidelity date boundaries managed via watermarks.
    - Change Detection: Structural delta management enabled via SHA2_256 `RowHash` bins.
    - Operational Logging: Automated logging capturing execution metrics:
        [TableName, StartTime, EndTime, DurationSeconds, RowsInserted, Status, ErrorMessage]

Prerequisites / Usage:
    - Run this entire script to build or reset the physical structures of the 
      Bronze Layer. Warning: Executing this drops existing tables and deletes data.
===================================================================================
*/
...
CREATE TABLE bronze.FactResellerSales (
    ...
    ProductStandardCost MONEY, TotalProductCost MONEY, SalesAmount MONEY, TaxAmt MONEY, Freight MONEY,
...
CREATE TABLE bronze.FactSalesQuota (
    ...
    SalesAmountQuota MONEY, Date DATETIME
)
...
CREATE TABLE bronze.DimCustomer (
    ...
    MiddleName NVARCHAR(50), LastName NVARCHAR(50), NameStyle BIT, BirthDate DATE, MaritalStatus NCHAR(1), Suffix NVARCHAR(10),
    Gender NVARCHAR(1), EmailAddress NVARCHAR(50), YearlyIncome MONEY, TotalChildren TINYINT, NumberChildrenAtHome TINYINT,
    ...
    AddressLine2 NVARCHAR(120), Phone NVARCHAR(20), DateFirstPurchase DATE, CommuteDistance NVARCHAR(15), RowHash BINARY(32) NULL
)
...
CREATE TABLE bronze.DimReseller (
    ...
    ResellerName NVARCHAR(50), NumberEmployees INT, OrderFrequency CHAR(1), OrderMonth TINYINT, FirstOrderYear INT,
    LastOrderYear INT, ProductLine NVARCHAR(50), AddressLine1 NVARCHAR(60), AddressLine2 NVARCHAR(60), AnnualSales MONEY,
    BankName NVARCHAR(50), MinPaymentType TINYINT, MinPaymentAmount MONEY, AnnualRevenue MONEY, YearOpened INT, RowHash BINARY(32) NULL
)
...
CREATE TABLE bronze.DimGeography (
    ...
    PostalCode NVARCHAR(15), SalesTerritoryKey INT, IpAddressLocator NVARCHAR(15), RowHash BINARY(32) NULL
)
...
CREATE TABLE bronze.DimSalesTerritory (
    ...
    SalesTerritoryGroup NVARCHAR(50), SalesTerritoryImage VARBINARY(MAX), RowHash BINARY(32) NULL
)
...
CREATE TABLE bronze.DimProduct (
    ...
    FinishedGoodsFlag BIT, Color NVARCHAR(15), SafetyStockLevel SMALLINT, ReorderPoint SMALLINT, ListPrice MONEY, Size NVARCHAR(50),
    SizeRange NVARCHAR(50), Weight FLOAT, DaysToManufacture INT, ProductLine NCHAR(2), DealerPrice MONEY, Class NCHAR(2), Style NCHAR(2),
    ...
    JapaneseDescription NVARCHAR(400), TurkishDescription NVARCHAR(400), StartDate DATETIME, EndDate DATETIME, Status NVARCHAR(7), RowHash BINARY(32) NULL
)
...
CREATE TABLE bronze.DimProductSubcategory (
    ...
    SpanishProductSubcategoryName NVARCHAR(50), FrenchProductSubcategoryName NVARCHAR(50),ProductCategoryKey INT, RowHash BINARY(32) NULL
)
...
CREATE TABLE bronze.DimProductCategory (
    ...
    FrenchProductCategoryName NVARCHAR(50), RowHash BINARY(32) NULL
)
...
CREATE TABLE bronze.DimEmployee (
    ...
    StartDate DATE, EndDate DATE, Status NVARCHAR(50), EmployeePhoto VARBINARY(MAX), RowHash BINARY(32) NULL
)
...
CREATE TABLE bronze.DimDate (
    ...
    CalendarSemester TINYINT, FiscalQuarter TINYINT, FiscalYear SMALLINT, FiscalSemester TINYINT, RowHash BINARY(32) NULL
)


-- 13. Table for Pipelone_log
...
-- 14. Table for Pipeline_Watermarks
